...
class MCTS:
    def __init__(
        ...
        if bench_num == 1:
            ...
            self.place_action = PlaceAction(scene_mngr, n_samples_held_obj=0, n_samples_support_obj=2, retreat_distance=0.2, n_directions=5)
    ...
    def _get_reward(self, cur_state:Scene=None, cur_logical_action:dict={}, next_state:Scene=None, depth=None, is_terminal:bool=False) -> float:
        ...
        if self.scene_mngr.scene.bench_num == 4:
            if logical_action_type == 'place':
                ...
                if not np.isclose(cur_obj_pose[1], peg_pose[1], 1e-01):
                    print(f"{sc.FAIL}Bad Action{sc.ENDC}")
                    reward = -3
                else:
                    reward = 1
    ...
    def get_all_joint_path(self, nodes):
        ...
        if self.scene_mngr.scene.bench_num == 2:
            ...
            pnp_path[-1][self.pick_action.move_data.MOVE_default_grasp] += self.scene_mngr.scene.ben_2_final_path
