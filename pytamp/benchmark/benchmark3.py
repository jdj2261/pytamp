...
class Benchmark3(Benchmark):
    ...
    def _load_objects(self):
        ...
        self.milk1_pose = Transform(pos=np.array([0.7, 0.48, self.table_height + self.clearbox_8_height + abs(self.milk1.bounds[0][2])]), rot=[0, 0, np.pi/12])
        self.milk2_pose = Transform(pos=np.array([0.62, 0.4, self.table_height + self.clearbox_8_height + abs(self.milk2.bounds[0][2])]), rot=[0, 0, -np.pi/12])
        self.milk3_pose = Transform(pos=np.array([0.6, 0.48, self.table_height + self.clearbox_8_height + abs(self.milk3.bounds[0][2])]), rot=[0, 0, np.pi/4])
        self.milk4_pose = Transform(pos=np.array([0.58, 0.32, self.table_height + self.clearbox_8_height + abs(self.milk3.bounds[0][2])]), rot=[0, 0, np.pi/6])
    ...
    def _load_scene(self):
        ...
        for i in range(self.rect_box_num):
            ...
            self.scene_mngr.add_object(name=rect_box_name, gtype="mesh", gparam=self.rect_boxes[i], h_mat=self.rect_box_poses[i].h_mat)

        for i in range(self.square_box_num):
            square_box_name = "square_box" + str(i)
            self.scene_mngr.add_object(name=square_box_name, gtype="mesh", gparam=self.square_boxes[i], h_mat=self.square_box_poses[i].h_mat)
