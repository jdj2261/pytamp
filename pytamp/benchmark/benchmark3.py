...
class Benchmark3(Benchmark):
    ...
    def _load_objects(self):
        ...
        self.clearbox_pose = Transform(pos=np.array([0.7, 0.4, self.table_height + abs(self.clearbox.bounds[0][2])]), rot=[0, 0, np.pi/2])
        ...
        for i in range(self.square_box_num):
            ...
            square_box_pose = Transform(
                pos=np.array([0.85, 0.4, self.table_height + self.clearbox_8_height + abs(self.square_boxes[0].bounds[0][2]) + squarebox_height * i]))
            square_box_color = [0, 0.2 + i * 0.1, .4]
        ...
        for i in range(self.rect_box_num):
            ...
            self.rect_boxes[i].apply_transform(get_h_mat(orientation=np.array([np.pi/2, 0, np.pi/2])))
            ...
            rect_box_pose = Transform(
                pos=np.array([0.72, 0.3, self.table_height + self.clearbox_8_height + abs(self.rect_boxes[i].bounds[0][2]) + rectbox_height * i]))
            
            rect_box_color = [0.4, 0.2 + i * 0.1, .4]
        ...
        self.milk1_pose = Transform(pos=np.array([0.76, 0.5, self.table_height + self.clearbox_8_height + abs(self.milk1.bounds[0][2])]), rot=[0, 0, np.pi/12])
        self.milk2_pose = Transform(pos=np.array([0.62, 0.42, self.table_height + self.clearbox_8_height + abs(self.milk2.bounds[0][2])]), rot=[0, 0, np.pi/20])
        self.milk3_pose = Transform(pos=np.array([0.6, 0.5, self.table_height + self.clearbox_8_height + abs(self.milk3.bounds[0][2])]), rot=[0, 0, -np.pi/2])
        self.milk4_pose = Transform(pos=np.array([0.58, 0.32, self.table_height + self.clearbox_8_height + abs(self.milk4.bounds[0][2])]))
        self.milk5_pose = Transform(pos=np.array([0.52, 0.42, self.table_height + self.clearbox_8_height + abs(self.milk5.bounds[0][2])]))
        self.can_pose = Transform(pos=np.array([0.72, 0.4, self.table_height + self.clearbox_8_height + abs(self.can.bounds[0][2])]))
    ...
    def _load_scene(self):
        ...
        for i in range(self.rect_box_num):
            ...
            self.scene_mngr.add_object(name=rect_box_name, gtype="mesh", gparam=self.rect_boxes[i], h_mat=self.rect_box_poses[i].h_mat, color=self.rect_box_colors[i])

        for i in range(self.square_box_num):
            square_box_name = "square_box" + str(i)
            self.scene_mngr.add_object(name=square_box_name, gtype="mesh", gparam=self.square_boxes[i], h_mat=self.square_box_poses[i].h_mat, color=self.square_box_colors[i])
        ...
        self.scene_mngr.add_object(name="clearbox", gtype="mesh", gparam=self.clearbox, h_mat=self.clearbox_pose.h_mat, color=[0.8, 0.8, 0.8])
