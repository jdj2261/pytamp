...
class Planner(NodeData, metaclass=ABCMeta):
    ...
    def _collide(self, new_q, only_robot=False, visible_name=False):
        """
        ...
        for link, transform in fk.items():
            if link in self._scene_mngr.robot_collision_mngr._objs:
                if self._scene_mngr.robot_collision_mngr.geom == "visual":
                    h_mat = np.dot(
                        transform.h_mat,
                        self._scene_mngr.scene.robot.links[link].visual.offset.h_mat,
                    )
                else:
                    h_mat = np.dot(
                        transform.h_mat,
        ...
        if is_attached:
            ...
            self._scene_mngr.robot_collision_mngr.set_transform(
                name=self._scene_mngr.attached_obj_name, h_mat=h_mat
            )
            self._scene_mngr.close_gripper()

        is_self_collision = self._scene_mngr.robot_collision_mngr.in_collision_internal(
            return_names=False
        )
    ...
    def _get_fk(self, q_in):
        ...
        if self.arm is not None:
            fk = self._scene_mngr.scene.robot.forward_kin(
                q_in, self._scene_mngr.scene.robot.desired_frames[self.arm]
            )
        else:
