...
for pick_actions in actions:
    for all_grasp_pose in pick_actions[pick.info.GRASP_POSES]:
        ...
        # pick.scene_mngr.render.render_axis(ax, all_grasp_pose[pick.move_data.MOVE_post_grasp])
        # pick.scene_mngr.render_gripper(ax, alpha=0.7, pose=all_grasp_pose[pick.move_data.MOVE_grasp])
copied_mesh = deepcopy(scene_mngr.scene.objs["hanoi_disk_2_6"].gparam)
copied_mesh.apply_transform(scene_mngr.scene.objs["hanoi_disk_2_6"].h_mat)

# print(scene_mngr.scene.objs["hanoi_disk_2_6"].h_mat)
center_point = copied_mesh.center_mass
test = np.eye(4)
test[:3, :3] = scene_mngr.scene.objs["hanoi_disk_2_6"].h_mat[:3, :3]
