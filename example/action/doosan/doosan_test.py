...
pick = PickAction(scene_mngr, n_contacts=0, n_directions=0, retreat_distance=0.1)
...
for obj in ["red_box", "blue_box", "green_box"]:
    ...
    for i in range(len(pose)):
        ...
        pick.scene_mngr.render.render_axis(ax, pose[i][pick.move_data.MOVE_pre_grasp])
        pick.scene_mngr.render.render_gripper(ax, robot, pose=pose[i][pick.move_data.MOVE_grasp])
...
# #         # pick.scene_mngr.render.render_gripper(ax, robot, pose=all_grasp_pose[pick.move_data.MOVE_grasp])
...
#             pick.scene_mngr.set_robot_eef_pose(ik_solve[pick.move_data.MOVE_grasp])
#             pick.scene_mngr.render_robot(ax)
