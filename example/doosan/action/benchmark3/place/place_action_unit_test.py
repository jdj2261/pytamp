...
surface_points_for_held_obj = list(place.get_surface_points_for_held_obj("can"))
...
eef_poses = list(pick.get_all_grasp_poses("can"))

# Add heuristic
eef_poses.extend(list(pick.get_grasp_pose_from_heuristic("can")))

all_release_poses = []
for eef_pose in eef_poses:
    release_poses = list(place.get_all_release_poses("clearbox_1_8", "can", eef_pose[pick.move_data.MOVE_grasp]))
    ...
    for release_pose, obj_pose in release_poses:
        ...
        place.scene_mngr.render.render_object(ax, place.scene_mngr.scene.objs["can"], obj_pose)
p_utils.plot_basis(ax)
