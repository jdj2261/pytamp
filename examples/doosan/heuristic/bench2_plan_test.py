...
from pytamp.search.plan import Plan
...
# for seed in range(10, 13):
    benchmark3 = Benchmark3(robot_name="doosan", geom="visual", is_pyplot=True, bottle_num=6)
    ...
    np.random.seed(seed)
    try:
        plan = Plan(benchmark3.scene_mngr)
        pnp_all_joint_path, pick_all_objects, place_all_object_poses = plan.do_planning(planner)
        
        num = 2
        filename = directory_name + '/benchmark3_test_algo({:})_budget({:})_seed({:})_{}.npy'.format(algo, budgets, seed, num)
