...
from pykin.utils.kin_utils import apply_robot_to_scene, apply_gripper_to_scene, apply_objects_to_scene

directory_name = 'bench4_scene/uct'
...
seed = 17
end_num = 0
filename = f'./bench4_planner/benchmark4_test_algo({method})_budget({budgets})_seed({seed})_{end_num}.npy'
        ...
        for _, (task, joint_path) in enumerate(pnp_joint_path.items()):
            for _, joint in enumerate(joint_path):
                ...
                if post_grasp_task_idx - grasp_task_idx == 1:
                    ...
                    attach_count += 1
                    is_attach = True
                    place.scene_mngr.attach_object_on_gripper(pick_all_object[attach_count-1], False)
                ...
                if post_release_task_idx - release_task_idx == 1:
                    ...
                    place.scene_mngr.add_object(name=pick_all_object[detach_count-1],
                                    gtype=place.scene_mngr.init_objects[pick_all_object[detach_count-1]].gtype,
                                    gparam=place.scene_mngr.init_objects[pick_all_object[detach_count-1]].gparam,
                                    h_mat=place_all_object_pose[detach_count-1],
                                    color=place.scene_mngr.init_objects[pick_all_object[detach_count-1]].color)
                    # place.scene_mngr.open_gripper(0.05)
                
                if is_attach == True:
                    place.scene_mngr.close_gripper(0.04)
                    trimesh_scene = apply_robot_to_scene(trimesh_scene=trimesh_scene, robot=place.scene_mngr.scene.robot, geom="visual")
                    trimesh_scene = apply_gripper_to_scene(trimesh_scene=trimesh_scene, robot=place.scene_mngr.scene.robot, geom="visual")
                else:
                    trimesh_scene = apply_robot_to_scene(trimesh_scene=trimesh_scene, robot=place.scene_mngr.scene.robot, geom="visual")
                trimesh_scene = apply_objects_to_scene(trimesh_scene=trimesh_scene, objs=place.scene_mngr.scene.objs)
                trimesh_scene.set_camera(np.array([-np.pi/2 - np.pi/3, np.pi, -np.pi/6]), 2.5, resolution=(640, 480))
                # data = trimesh_scene.save_image(visible=False)
                data = trimesh_scene.save_image(resolution=[480, 360], visible=True)

                data_io = io.BytesIO(data)
                # img = Image.open(data_io)
                im = Image.open(data_io)
                file_name = directory_name + '/bench4_'+str(idx)+'.png'
                im.save(file_name)
