...
directory_name = 'bench2_scene/random'
...
benchmark2 = Benchmark2(robot_name="doosan", geom="visual", is_pyplot=False, bottle_num=6, is_only_render=True)
    ...
    for pnp_joint_path in pnp_joint_all_path:    
        for _, (task, joint_path) in enumerate(pnp_joint_path.items()):
            for _, joint in enumerate(joint_path):
                ...
                trimesh_scene.set_camera(np.array([-np.pi/2 - np.pi/6, np.pi, np.pi/6 + np.pi/2]), 3, resolution=(640, 480))
                ...
                file_name = directory_name + '/bench2_'+str(idx)+'.png'
                im.save(file_name)
