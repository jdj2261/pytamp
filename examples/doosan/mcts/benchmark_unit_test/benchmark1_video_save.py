...
for idx, c in enumerate(c_list):
    ...
    for i in range(mcts.budgets):
        ...
        if mcts.level_wise_2_success and not mcts.has_aleardy_level_1_optimal_nodes:
            ...
            num = 0
            file_name = directory_name + '/benchmark1_test_algo({:})_budget({:})_seed({:})_iter({:})_{}.mp4'.format(algo, budgets, seed, i, num)
            ...
            mcts.place_action.simulate_path(pnp_all_joint_path, pick_all_objects, place_all_object_poses, is_save=True, video_name=file_name)