...
pick = PickAction(benchmark4.scene_mngr, n_contacts=0, n_directions=0, retreat_distance=0.2)
place = PlaceAction(benchmark4.scene_mngr, n_samples_held_obj=0, n_samples_support_obj=0, retreat_distance=0.5, release_distance=0.01)

pick_action = pick.get_action_level_1_for_single_object(pick.scene_mngr.scene, "hanoi_disk_2")
...
for pick_scene in pick.get_possible_transitions(pick.scene_mngr.scene, pick_action):
    ...
    if pick_joint_path:
        place_action = place.get_action_level_1_for_single_object("table", "hanoi_disk_2", pick_scene.robot.gripper.grasp_pose, scene=pick_scene)
