...
surface_points_for_held_obj = list(place.get_surface_points_for_held_obj("hanoi_disk_0"))
...
for eef_pose in eef_poses:
    release_poses = list(place.get_all_release_poses_and_obj_pose("table", "hanoi_disk_0", eef_pose[pick.move_data.MOVE_grasp]))
    ...
    for release_pose, obj_pose in release_poses:
        ...
        place.scene_mngr.render.render_object(ax, place.scene_mngr.scene.objs["hanoi_disk_0"], obj_pose)
p_utils.plot_basis(ax)
...
for release_pose_for_only_gripper, obj_pose in release_poses_for_only_gripper:
    ...
    pick.scene_mngr.render.render_gripper(ax, benchmark4.robot, pose=release_pose_for_only_gripper[place.move_data.MOVE_release])
