...
place = PlaceAction(benchmark3.scene_mngr, n_samples_held_obj=0, n_samples_support_obj=3, n_directions=10)


###### Surface sampling held and support obj#######
fig, ax = p_utils.init_3d_figure(figsize=(10,6), dpi=120, name="Sampling Object")
surface_points_for_support_obj = list(place.get_surface_points_for_support_obj("clearbox_1_8"))
for point, normal, _ in surface_points_for_support_obj:
    place.scene_mngr.render.render_point(ax, point)
surface_points_for_held_obj = list(place.get_surface_points_for_held_obj("rect_bottom_box"))
...
for eef_pose in eef_poses:
    release_poses = list(place.get_all_release_poses_and_obj_pose("clearbox_1_8", "rect_bottom_box", eef_pose[pick.move_data.MOVE_grasp]))
    ...
    for release_pose, obj_pose in release_poses:
        ...
        place.scene_mngr.render.render_object(ax, place.scene_mngr.scene.objs["rect_bottom_box"], obj_pose)
p_utils.plot_basis(ax)
