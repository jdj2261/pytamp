...
surface_points_for_held_obj = list(place.get_surface_points_for_held_obj("rect_box"))
...
eef_poses = list(pick.get_all_grasp_poses("rect_box"))

# Add heuristic
eef_poses.extend(list(pick.get_grasp_pose_from_heuristic("rect_box")))

all_release_poses = []
for eef_pose in eef_poses:
    release_poses = list(place.get_all_release_poses_and_obj_pose("clearbox_1_8", "rect_box", eef_pose[pick.move_data.MOVE_grasp]))
    ...
    for release_pose, obj_pose in release_poses:
        ...
        place.scene_mngr.render.render_object(ax, place.scene_mngr.scene.objs["rect_box"], obj_pose)
p_utils.plot_basis(ax)
