...
surface_points_for_support_obj = list(place.get_surface_points_for_support_obj("shelf_9"))
for point, normal, _ in surface_points_for_support_obj:
    place.scene_mngr.render.render_point(ax, point)
surface_points_for_held_obj = list(place.get_surface_points_for_held_obj("goal_bottle"))
...
eef_poses = list(pick.get_all_grasp_poses("goal_bottle"))

# Add heuristic
eef_poses.extend(list(pick.get_grasp_pose_from_heuristic("goal_bottle")))

all_release_poses = []
for eef_pose in eef_poses:
    release_poses = list(place.get_all_release_poses_and_obj_pose("shelf_9", "goal_bottle", eef_pose[pick.move_data.MOVE_grasp]))
    pick.scene_mngr.render.render_gripper(ax, benchmark2.robot, pose=eef_pose[place.move_data.MOVE_grasp])
    for release_pose, obj_pose in release_poses:
        ...
        place.scene_mngr.render.render_gripper(ax, benchmark2.robot, pose=release_pose[place.move_data.MOVE_release])
        ...
        place.scene_mngr.render.render_object(ax, place.scene_mngr.scene.objs["goal_bottle"], obj_pose)
p_utils.plot_basis(ax)
...
for release_pose_for_only_gripper, obj_pose in release_poses_for_only_gripper:
    ...
    pick.scene_mngr.render.render_gripper(ax, benchmark2.robot, pose=release_pose_for_only_gripper[place.move_data.MOVE_release])
