...
for eef_pose in eef_poses:
    release_poses = list(
        place.get_all_release_poses_and_obj_pose(
            "shelf_9", "goal_bottle", eef_pose[pick.move_data.MOVE_grasp]
        )
    )
    pick.scene_mngr.render.render_gripper(
        ax, benchmark2.robot, pose=eef_pose[place.move_data.MOVE_grasp]
    )
...
for release_pose_for_only_gripper, obj_pose in release_poses_for_only_gripper:
    ...
    pick.scene_mngr.render.render_gripper(
        ax, benchmark2.robot, pose=release_pose_for_only_gripper[place.move_data.MOVE_release]
    )
